...
**Ampliación aceptada el 22 de septiembre de 2026:** el responsable autoriza el mismo diagnóstico, targets y tratamiento en Xcode 27.2 beta (`27B5019j`), tras comprobar su repetición en las compilaciones iOS y servidor. Se conserva también la aceptación de Xcode 27.0 (`27A266a`); no se amplía a otros avisos ni se oculta el mensaje.

- **Diagnóstico permitido:** `Metadata extraction skipped, no AppIntents.framework dependency found`, emitido por `appintentsmetadataprocessor` de Xcode 27.0 (`27A266a`) o Xcode 27.2 beta (`27B5019j`).
- **Alcance:** exclusivamente ese mensaje en los targets `SmartShoppingList`, `SmartShoppingListTests` y el bundle de pruebas de `SmartShoppingListServer`, mientras no utilicen App Intents. No abarca otros diagnósticos, versiones de Xcode distintas de las enumeradas ni errores de extracción de una funcionalidad real.
...
- **Recomprobación del 22 de septiembre con Siri habilitada:** el usuario comunicó que Siri AI ya está activa. El log completo consultado mediante Xcode MCP conserva el diagnóstico emitido a las 13:08:36 por `appintentsmetadataprocessor` para `SmartShoppingList`. El código propio no declara App Intents ni importa AppIntents; la entrada de voz usa Speech y la interpretación usa Foundation Models. Activar Siri no añade una dependencia ni metadatos al target. Se mantiene EXC-002, sin incorporar el framework únicamente para eliminar el aviso. [Apple documenta que los metadatos se generan durante la compilación de los intents declarados](https://developer.apple.com/documentation/appintents).
