...
Pendientes: formatos regionales UK/USA, revisión de entrega y seguimiento de los cierres de Foundation Models observados en previews. Los hallazgos de visibilidad de avisos, foco y anuncio de errores conservan su seguimiento de fase 3; no se consideran resueltos por traducir la interfaz ni por el ensayo acotado de VoiceOver.
...
Los textos visibles —incluidos `LocalizedStringResource` e interpolaciones— cambian de idioma; los productos de ejemplo españoles se conservan en inglés. No se identificaron nuevos recortes o solapamientos en las áreas inspeccionadas. El placeholder abreviado es visible en AX 5. Evidencia local de renders del 24 de septiembre, 00:18–00:21: `/tmp/ssl-preview-add-{en,es}-{large,xxx,ax5}.json`, `/tmp/ssl-preview-purchase-{en,es}-large.json` y `/tmp/ssl-preview-review-{en,es}-large.json`, con las rutas de imagen devueltas por Xcode.
...
2. El recorrido manual inglés en iPhone 11 y las etiquetas de VoiceOver del recorrido indicado en iPhone 14 están confirmados arriba. Sigue pendiente revisar fechas y formatos con región UK/USA; los datos españoles deben seguir apareciendo tal como se introdujeron.
