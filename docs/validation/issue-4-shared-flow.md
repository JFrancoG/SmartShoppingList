...
## Corrección del middleware autorizada — 22 de septiembre

Tras publicar la revisión en `6bc4cf3`, el usuario autorizó corregir el middleware antes de continuar con los enlaces. `APIErrorMiddleware` pasa al final de la lista global, dentro del manejador genérico de Vapor, para transformar el error original en la respuesta contractual antes de que el genérico lo intercepte. Se conservan la autenticación, las transacciones, los pins y el middleware de las rutas de compras.

Se añadieron seis pruebas Swift Testing (ocho casos) en la suite PostgreSQL existente, atravesando `app.sendRequest` y la configuración real: `/v1/me` y logout sin bearer; bodies inválidos en challenge y login; sesión revocada mediante logout real; sesión caducada; body de más de 128 KiB; ruta API inexistente. Verifican status, Content-Type, código, mensaje acotado e ID de petición, sin aceptar el formato genérico `error/reason`. Las sesiones son sintéticas y viven únicamente en la base de pruebas.

- **RED:** antes de corregir producción, los ocho casos nuevos fallaron y las 98 ejecuciones previas pasaron. Bundle nativo `Test-SmartShoppingListServer-2026.09.22_16-33-55-+0200.xcresult`: 61 pruebas únicas aprobadas y seis fallidas. El primer intento de registro del archivo mediante MCP falló y Xcode ejecutó solo las pruebas anteriores; ese pase no se cuenta como RED. Las regresiones quedaron incorporadas al archivo existente de la suite y el siguiente pase sí las ejecutó.
- **GREEN:** Xcode 27.2 beta, macOS 27.2 arm64, PostgreSQL aislado: **67 pruebas únicas / 106 ejecuciones, cero fallos, cero omitidas y cero warnings de runtime**. Bundle nativo cerrado: `Test-SmartShoppingListServer-2026.09.22_16-34-43-+0200.xcresult`, bajo `DerivedData/SmartShoppingListServer-gglgzaxldexmsodyegvupaqgkklo/Logs/Test/`. Se conserva únicamente el aviso de metadatos App Intents observado en el log, cubierto por EXC-002.
- Revisión manual y script de estilo de los dos Swift modificados sin hallazgos; `git diff --check` correcto. No cambian fuentes iOS, por lo que no se repite su suite ni se atribuye nueva validación física del cliente.

- **Docker Release Linux arm64: PASS**, imagen `smartshoppinglistserver:auth-middleware-fix`, ID `sha256:11fe968de587890513156e72fc6a10a697e85a86f55e24a3895c5b4b47cd1539`, usuario `vapor:vapor`. Log: `/tmp/smartshoppinglist-sep22-review/auth-linux-build.log`. Este build reutiliza la capa de resolución existente; no vuelve a certificar esa resolución sin el warning aceptado de JWTKit.
- **HTTP real sobre la imagen corregida: PASS**, con aserciones estrictas de status y formato de error (sin tolerar los fallos conocidos de la revisión). Pasan los cinco casos negativos de autenticación anteriores y, además, sesión caducada, body excesivo y ruta API inexistente. También se repite satisfactoriamente el recorrido de grupo/invitaciones, aislamiento, Unicode, rollback, replay, reinicio y logout. Script y logs: `/tmp/smartshoppinglist-sep22-review/auth-smoke.py`, `auth-smoke.log`, `auth-runtime.log`. Contenedor y base temporal retirados; `db-test` vuelve a quedar detenido. No se repite la suite Swift Testing completa dentro de Linux: las 67 pruebas / 106 ejecuciones nuevas corresponden a Xcode/macOS, y la imagen Linux se comprueba por HTTP real. No se acredita amd64 ni despliegue Railway nuevo.

El usuario autorizó publicar la corrección y esta evidencia mediante commit y push. El commit remoto verificado se registra en #4 y PR #9 después de la publicación. Quedan pendientes el despliegue Railway de la corrección y los ensayos alojados. El siguiente ensayo manual sigue siendo el de enlaces; esta corrección no completa #7 ni cierra la PR o las issues.

## Enlaces por Mail/Mensajes y servidor alojado — 22 de septiembre

El usuario confirma que el enlace recibido por **Mail y Mensajes** abre correctamente la app y muestra la invitación en el iPhone 11. El enlace se generó con la cuenta creadora del iPhone 11 y se trasladó como texto mediante una nota/AirDrop al iPhone 14 para enviarlo desde allí, por las restricciones de envío del teléfono de trabajo. Ambos usuarios ya pertenecían al grupo: acredita transporte y apertura de la invitación, no una incorporación nueva ni consumo del enlace. AirDrop ya estaba acreditado en los ensayos anteriores.

El usuario confirma además que el último deployment terminó con éxito. Comprobación directa por HTTPS del 22 de septiembre a las 15:40 UTC (17:40 CEST): `GET https://smartshoppinglist-production.up.railway.app/v1/me` sin bearer devuelve **HTTP 401**, `Content-Type: application/json; charset=utf-8`, código `invalid_session`, mensaje de reautenticación y `requestId` UUID. Queda confirmado este comportamiento corregido en el alojamiento; la respuesta no identifica por sí sola el SHA desplegado y no sustituye la suite completa ni los ensayos de sesiones revocadas/caducadas.

El primer ensayo de enlace alterado fue **inconcluso**: el usuario comunica que cambiar un carácter, incluso con una invitación nueva, abre la app con Aceptar/Descartar y sin aviso. Descartar retira la presentación sin mostrar error. La revisión del código confirma que una invitación pendiente distinta conserva prioridad sobre la entrante y que el servidor compara identificador y hash del token; eso no establece qué URL recibió el dispositivo. Se decidió aislar recepción del enlace y estado previo con una URL sintética distinta, sin credenciales reales; los ensayos posteriores se describen a continuación. No se acredita aceptación de un token incorrecto ni se atribuye todavía una causa al comportamiento.

El ensayo controlado posterior con URL sintética (`/invite/abcdef01-0000-4000-8000-000000000001` y token sintético canónico) sí muestra, según confirma el usuario, «El recurso no está disponible o no tienes acceso. Actualiza e inténtalo de nuevo», sin Aceptar ni Descartar. Acredita recepción/apertura y presentación del rechazo para esa invitación inexistente en el iPhone 11. No acredita todavía el rechazo de un token alterado asociado al identificador de una invitación real ni determina por qué el ensayo anterior mostraba una vista previa válida.

El usuario confirma después los dos rechazos sobre un enlace con identificador de invitación real: (1) token de 43 `a` minúsculas → «El enlace de invitación no es válido para esta app», coherente con el rechazo local del formato no canónico; (2) token de 43 `A` mayúsculas → «El recurso no está disponible o no tienes acceso. Actualiza e inténtalo de nuevo», coherente con el rechazo del servidor a un secreto incorrecto de formato válido. Queda acreditado el rechazo alojado del enlace alterado mediante el resultado comunicado por el usuario; no se afirma haber observado directamente la petición HTTP ni se atribuye una causa confirmada al primer ensayo inconcluso. No se guarda el enlace real ni su secreto en el informe.

### Límites de validación y continuidad acordados

- **Caducidad:** ensayo alojado pendiente. El usuario revocó los enlaces antiguos; estos no permiten aislar la caducidad. Se indicó crear y conservar una invitación nueva sin aceptar ni revocar, y abrirla después de más de 24 horas. No consta todavía la hora de creación ni la confirmación de que se guardó; no se fija una hora de ensayo ni se marca aprobada por el mero cambio de día.
- **Consumida por otra identidad:** cubierta por pruebas automatizadas locales, pero sin ensayo manual alojado. El enlace original de incorporación se compartió por AirDrop y el usuario no lo conserva. Las dos cuentas ya pertenecen al grupo: una invitación nueva no reproduce una incorporación que consuma el enlace. No se resetea la base ni se cambia la pertenencia para forzar esta prueba.
- **IA y envío compartido:** se acreditan por separado interpretación real → borrador en Mac y borrador revisado → envío al grupo → consulta en segundo dispositivo mediante entrada manual. El envío utiliza los valores revisados y no reinterpreta el texto. El usuario acepta continuar sin repetir ahora la cadena completa: se considera una comprobación adicional de integración, no un bloqueo para iniciar compra. No se afirma que ese recorrido completo se haya ejecutado, ni se dan por aprobados inglés o los pendientes de #7.
